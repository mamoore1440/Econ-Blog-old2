<template>
  ...
  >
    <nuxt-link ref="title" :to="`/tags/${tagGroup}/${tag.slug}`" class="font-bold text-2xl hover:underline">
      {{tag.title}}
    </nuxt-link>
    ...
    <br>
      ...
      <div v-if="latestPosts.length" class="relative">
        ...
        <div v-if="latestPosts.length > 1">
          ...
          <nuxt-link ref="view-tag" :to="`/tags/${tagGroup}/${tag.slug}`" class="text-extra-gray-dark dark:text-extra-gray-light text-md underline hover:no-underline transition">
            View all posts
          </nuxt-link>
        </div>
      </div>
