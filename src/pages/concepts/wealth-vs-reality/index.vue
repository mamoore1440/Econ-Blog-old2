<template>
  <main>
    ...
    <back-to-top-button />
    <footer-bar current-page="Concepts" />
  </main>
</template>
...
export default {
  ...
  async asyncData({ $content, params, error }) {
    const post = (await $content('concepts/wealth-vs-reality')
      .where({ slug: "wealth-vs-reality" })
      .limit(1)
      .fetch()
      .catch((err) => {
        error({
